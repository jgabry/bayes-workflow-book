...
$2\Phi(\sin^{-1}((R-r)/x) / \sigma) - 1$, where $\Phi$ is the cumulative normal
...
- Expanding the model to allow for the possibility that more people
...
For simplicity, we're pretending ratings are continuous unbounded
numbers (rather than integers from 0 through 5) and that the
distributions of ratings for the two movies differ only in their mean,
...
We will assume that movies typically get ratings of
...
Here is the resulting Stan program:
...
The estimated population average popularity is 3.7 for the first movie
and 4.0 for the second.  We have more information on the second movie,
...
To keep things simple, we will ignore
...
We can check the fit by plotting the posterior inferences against the true 
parameter values:
...
Roughly half of the 50% intervals and 95% of the 95% intervals contain
...
study?].  The new models are equivalent, with the movie quality
...
We'll start by simulating data from $J=40$ movies and $K=100$ raters,
...
We then plot true movie quality (the parameter $\alpha_j$), first versus raw
...
### Other potential extensions to the model {-}
...
it by a normal, or keeping the $t$ but estimating the degrees of
freedom parameter, did not change anything noticeably.  Our next idea
was that the discreteness of the data could be causing the problem.
...
Aha! The lines are fanning out from a common starting point. We'll fit a
regression to each line and then summarize each line by its average slope.  This
is not necessarily the most efficient way to estimate a slope from correlated
data, but we will not be concerned about that, as our purpose here is to
demonstrate decision analysis in the context of a fitted model, rather than to
fit the model in an optimal way.
...
```{r}
hist(slope, xlab="Slope", breaks=seq(floor(10*min(slope)), 
                                     ceiling(10*max(slope)))/10)
```
...
data_graph <- function(a, b, sigma, x, y) {
  curve(a[1]*exp(-b[1]*x) + a[2]*exp(-b[2]*x), from=min(x), 
        to=max(x), ylim=c(0, 1.05*max(y)), xlim=c(0, max(x)),
  ...
  text(max(x), 0.5*max(y), 
       paste("y = ", fround(a[1], 1), "*exp(", fround(-b[1], 1), "*x) + ", 
             fround(a[2], 1), "*exp(", fround(-b[2], 1), "*x)", sep=""), 
       adj=1)
}
...
tell them apart based on noisy data, even 1000 measurements.  So Stan had
trouble recovering the true parameters from the data.
...
```{r, results=FALSE}
fit_2_with_priors <- stan("stan/sum_of_exponentials_with_priors.stan", 
                          data=data_2)
```
```{r}
