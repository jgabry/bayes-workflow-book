...
```
Given we have generated only 5 data points and have supplied the prior information that the mean is around 10 we observe the following posterior distribution.
```{r, results = F}
set.seed(24)
y <- rnorm(5,100,2)
N <- length(y)
...
lines(density(rnorm(N,
      ...
      col = "green")
```
Evidently the likelihood becomes more informative and the posterior distribution moves much closer to the likelihood. Given that we know the actual mean of $y$ is 100 and the standard deviation is 2, we can infer that our posterior is very closely approximating the true data generating process. It can be seen in the above example that each additional data point adds another term to the likelihood in the implicit vector addition of the `target +=` or `~` statement.  See Chapter 4 of _Bayesian Data Analysis_ for more on
this. 
...
## Problems with so-called noninformative priors [objective - Prior information not included in models!]
