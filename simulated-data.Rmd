...
- Choose values for all _constants_ in the target model
- Simulate _parameters_ from the prior
- Simulate the _data_ from the parameters
...
The variables in this model consists of
- a constant integer value `N`
- parallel data vectors `x` and `y` of the predictor and observed outcome values.
- regression parameters `a`, `b`, and `sigma`

To simulate data for this model, first we choose a positive integer value for `N`.  In this example, we set $N = 50$.

To simulate the parameters from the prior, we need to:
- understand default prior
- have access to a function which generates random values according to a specified probability distribution.

************


The program doesn't specify priors on any of the parameters, therefore
the Stan uses uniform(-infinity, +infinity) as the default prior.
...
- simulating data for hierarchical models:  You can choose assumed values for the hyperparameters but you should draw the lower-level parameters from the model

- Choosing or drawing random values for unmodeled data such as N and x

- What if simulating data gives a bad answer, if the model cannot recover the assumed parameter values?